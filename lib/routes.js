...
import { hashPassword, checkPassword, sessionCookie, clearedCookie, readSession, publicUser } from './auth.js';
...
on('GET', '/api/products', STAFF, async ({ db, res, query }) => {
  ...
  const r = await db.query(
    `select * from stock_summary
      where sku ilike $1 or name ilike $1 or coalesce(brand, '') ilike $1
      order by name`, [term]);
  ok(res, r.rows);
});
